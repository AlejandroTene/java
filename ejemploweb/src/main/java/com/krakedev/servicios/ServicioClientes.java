...
public class ServicioClientes {
	...
	public Cliente buscar() {
		Cliente cliente=new Cliente("1163957462","Carlos",1);
	...
	@Path("buscarPorNumeroHijos/{numeroParam}")
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response obtenerClientesPorNumeroHijos(@PathParam("numeroParam") int numeroHijos){
		ClientesBDD cli=new ClientesBDD();
		ArrayList<Cliente> clientes=null;
		try {
			clientes = cli.buscarClientesPorNH(numeroHijos);
			return Response.ok(clientes).build();
		} catch (KrakeDevException e) {
			e.printStackTrace();
			return Response.serverError().build();
